use skillcapped_generator::{app::App, terminal::{setup_terminal, restore_terminal, run_app, update_copy_feedback}};
use ratatui::{backend::TestBackend, Terminal, backend::CrosstermBackend};
...
fn test_setup_and_restore_terminal() {
    ...
    let _setup_fn: fn() -> Result<Terminal<CrosstermBackend<std::io::Stdout>>, Box<dyn std::error::Error>> = setup_terminal;
    let _restore_fn: fn(&mut Terminal<CrosstermBackend<std::io::Stdout>>) -> Result<(), Box<dyn std::error::Error>> = restore_terminal;
}
