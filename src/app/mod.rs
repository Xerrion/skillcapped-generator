...
impl App {
    ...
    fn decode_import_string(&self, encoded_string: &str) -> Result<String, String> {
        let cleaned: String = encoded_string.chars()
    ...
    pub fn get_wa_configs(&self) -> (String, String) {
        let wa4_part1: String = vec![99, 116, 100, 118, 101, 105]
            .into_iter().map(|c| c as u8 as char).collect();
...
mod tests {
    ...
    fn test_validate_code() {
        ...
        let wrong_content = base64::engine::general_purpose::STANDARD.encode("wrong_content");
        assert!(!app.validate_code(&wrong_content));
    }

    #[test]
    fn test_decode_import_string() {
        let app = App::new();
        let test_string = "Hello World";
        let encoded = base64::engine::general_purpose::STANDARD.encode(test_string);
        
        let result = app.decode_import_string(&encoded);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), test_string);
        
        // Test with invalid base64
        let result = app.decode_import_string("invalid_base64!@#");
        assert!(result.is_err());
    }

    #[test]
    fn test_get_addon_config() {
        let mut app = App::new();
        
        app.version = "retail".to_string();
        let result = app.get_addon_config();
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), "vridtcetvrdice");
